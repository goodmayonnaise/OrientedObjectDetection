...
class RotatedMSDCNHead(BaseDenseHead):
    """YOLOv8 Head"""
    def __init__(self,
                 num_classes: int,
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_levels):
            ...
            self.ang_preds.append(
                nn.Sequential(
                    ConvModule(in_channels=3*self.in_channels[i]//8,
                               out_channels=ang_out_channels,
                               kernel_size=3,
                               stride=1,
                               padding=1,
                               norm_cfg=self.norm_cfg,
                               act_cfg=self.act_cfg),
                    ConvModule(in_channels=ang_out_channels,
                               out_channels=ang_out_channels,
                               kernel_size=3,
                               stride=1,
                               padding=1,
                               norm_cfg=self.norm_cfg,
                               act_cfg=self.act_cfg),
                    nn.Conv2d(in_channels=ang_out_channels,
