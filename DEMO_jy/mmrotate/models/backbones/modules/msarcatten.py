...
from .adaptive_rotated_conv import batch_rotate_multiweight, AdaptiveRotatedConv2d
...
class MSARCModule(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if not isinstance(x, list):
            _,c,_,_ = x.shape
            x = list(x.split(c//self.num_conv, 1))
...
class CSPLayerWithMSARCAtten(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for i, block in enumerate(self.blocks):
            ...
            if self.num_block > 1:
                m_c= int(learning_feats[1].size(1)*self.expand_ratio)
                split_feat = torch.cat([l_f[:,:m_c,:,:] for l_f in learning_feats], dim=1)
                learning_feats = [l_f[:,m_c:,:,:] for l_f in learning_feats]
                out_list.append(split_feat)
                out_list.append(torch.cat(learning_feats, dim=1)) if i == self.num_block-1 else None
            else:
                out_list.append(torch.cat(learning_feats, 1))
...
class MSARCModuleLarge(nn.Module):
    def __init__(self,
                 dim: int,
                 dilation: Sequence[int] = (1, 1, 1),
                 padding: Sequence[int] = (1, 2, 3),
...
class CSPLayerWithMSARCAttenLarge(nn.Module):
    def __init__(
            self,
            in_channels: int,
            out_channels: int,
